...
public class VinylSimulation implements PropertyChangeListener {
      ...
      for (int i = 0; i < 1000; i++) {
        ...
        try {
          if (random == 1) {
            ...
            System.out.println(
                customers[i % 2].getName() + " borrowed " + vinyl.getTitle()
                    + " which is now: " + vinyl.getLendingStatus());
          }
          else if (random == 2) {
            model.reserveVinyl(vinyl, customers[i % 2]);
            System.out.println(
                customers[i % 2].getName() + " reserved " + vinyl.getTitle()
                    + " which is now: " + vinyl.getLendingStatus());
          }
          else {
            model.returnVinyl(vinyl, customers[i % 2]);
            System.out.println(
                customers[i % 2].getName() + " returned " + vinyl.getTitle()
                    + " which is now: " + vinyl.getLendingStatus());
          }
        }
        catch (Exception e) {
  ...
  @Override public void propertyChange(PropertyChangeEvent event) {
    //to be edited
    System.out.println(event.getPropertyName() + " " + event.getOldValue() + " "
        + event.getNewValue());
  }
}
